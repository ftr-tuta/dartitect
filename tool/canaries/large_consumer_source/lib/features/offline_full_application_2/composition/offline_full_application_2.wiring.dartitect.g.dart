...
/// Exact contexts selected by the OfflineFullApplication2 feature profile.
final class OfflineFullApplication2Infrastructure {
  const OfflineFullApplication2Infrastructure({
...
final class OfflineFullApplication2Runtime {
  const OfflineFullApplication2Runtime({
  ...
  final OfflineFullApplication2Factory factory;
  final OfflineFullApplication2Infrastructure infrastructure;
  ...
  static Future<OfflineFullApplication2Runtime> create(
    ApplicationGraph graph,
    OfflineFullApplication2Factory factory,
    ResourceTransaction transaction,
  ) async {
    final infrastructure = OfflineFullApplication2Infrastructure(
    ...
    final syncEngine = transaction.own<SyncEngine<String, int, LargeFailure>>(
      ...
      label: 'feature.offline_full_application_2.sync',
    );
    final mutationCommand = transaction
        .own<MutationCommand<LargeMutation, String, void, LargeFailure>>(
          ...
          (value) => value.disposeAsync(),
          label: 'feature.offline_full_application_2.mutations',
        );
    final repository = transaction.own<LargeRepository>(
      ...
      (value) => value.disposeAsync(),
      label: 'feature.offline_full_application_2.repository',
    );
    return OfflineFullApplication2Runtime(
      factory: factory,
...
/// Material-neutral generated owner for the OfflineFullApplication2 feature.
final class OfflineFullApplication2FeatureHost extends StatelessWidget {
  const OfflineFullApplication2FeatureHost({
    required this.graph,
    required this.factory,
  ...
  final FeatureReadyBuilder<OfflineFullApplication2Runtime, LargeViewModel>
  ready;
  ...
  Widget build(BuildContext context) =>
      FeatureHost<
        ApplicationGraph,
        OfflineFullApplication2Runtime,
        LargeViewModel
      >(
        parent: graph,
        generationKey: factory,
        createGraph: (parent, transaction) =>
            OfflineFullApplication2Runtime.create(parent, factory, transaction),
...
abstract final class OfflineFullApplication2FeatureWiring {
