...
/// Exact contexts selected by the ReplicaSession3 feature profile.
final class ReplicaSession3Infrastructure {
  const ReplicaSession3Infrastructure({
...
final class ReplicaSession3Runtime {
  const ReplicaSession3Runtime({
  ...
  final ReplicaSession3Factory factory;
  final ReplicaSession3Infrastructure infrastructure;
  ...
  static Future<ReplicaSession3Runtime> create(
    SessionGraph graph,
    ReplicaSession3Factory factory,
    ResourceTransaction transaction,
  ) async {
    final infrastructure = ReplicaSession3Infrastructure(
    ...
    final syncEngine = transaction.own<SyncEngine<String, int, LargeFailure>>(
      ...
      label: 'feature.replica_session_3.sync',
    );
    final repository = transaction.own<LargeRepository>(
      ...
      (value) => value.disposeAsync(),
      label: 'feature.replica_session_3.repository',
    );
    return ReplicaSession3Runtime(
      factory: factory,
...
/// Material-neutral generated owner for the ReplicaSession3 feature.
final class ReplicaSession3FeatureHost extends StatelessWidget {
  const ReplicaSession3FeatureHost({
    required this.graph,
    required this.factory,
  ...
  final FeatureReadyBuilder<ReplicaSession3Runtime, LargeViewModel> ready;
  final FeatureViewModelStarter<LargeViewModel>? start;
  final FutureOr<void> Function()? onDisposed;

  @override
  Widget build(BuildContext context) =>
      FeatureHost<SessionGraph, ReplicaSession3Runtime, LargeViewModel>(
        parent: graph,
        generationKey: factory,
        createGraph: (parent, transaction) =>
            ReplicaSession3Runtime.create(parent, factory, transaction),
...
abstract final class ReplicaSession3FeatureWiring {
