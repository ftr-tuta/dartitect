...
/// Checks the same-host legacy/RC8 modeling comparison and hard resource budgets.
void main() {
  ...
  if (artifact['schemaVersion'] != 2 ||
      artifact['targetVersion'] != '1.0.0-rc.9' ||
      policy['coldRuns'] != 5 ||
      policy['warmRuns'] != 20 ||
      policy['maxRegressionPercent'] != 10.0 ||
      policy['cacheAuthority'] != false ||
      policy['sameHostRequired'] != true) {
    errors.add('Model benchmark policy is not the frozen RC8 contract.');
  }
  ...
  stdout.writeln(
    'Model benchmark passes: same-host legacy/RC8, five cold medians/RSS, '
