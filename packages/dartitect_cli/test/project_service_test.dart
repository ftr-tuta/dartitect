...
''');
      ...
      final plan = await service.previewDependencyUpgrade('1.2.0');
      ...
      expect(plan.targetCohort, '1.2.0');
      expect(receipt.changed, isTrue);
...
''');
    await expectLater(
      service.previewDependencyUpgrade('1.2.0'),
      throwsA(
        isA<DartitectChangeException>().having(
          (error) => error.code,
          'code',
...
''';
    ...
    expect(receipt.changed, isTrue);
    expect(await pubspec.readAsString(), contains('version: 1.2.0'));
    expect(
