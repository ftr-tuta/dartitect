...
The only old-manifest upgrade is for a project generated by the supported
pre-stable modeling generator. Its `.dartitect/model-outputs.json` schema 1 is
migrated atomically only when the recorded generator identity and every current
canonical output digest match the migration contract. The earlier output
journal schema 2 and source journal schema 1 are recoverable at their original
paths. Any mismatch, ambiguous old/new artifact pair, downgrade, or force
attempt fails closed without writes.
