...
The only old-manifest upgrade is for a project already generated by Dartitect
RC3: `.dartitect/model-outputs.json` schema 1 is migrated atomically only when its
generator identity is exactly `1.0.0-rc.3` and every current canonical output
digest matches. The earlier output journal schema 2 and source journal schema 1
